...
export interface DashboardDetail extends Omit<SelectDashboard, 'userId' | 'deletedAt'> {
  pages: Array<{
    id: string;
    name: string;
    icon: string | null;
  ...
  .handler(
    async ({ data, context }): Promise<ApiResponse<DashboardDetail>> => {
      try {
        ...
        if (!baseSlug) {
          throw new ValidationError('Could not generate a valid slug from the dashboard name');
        }
  ...
  .handler(
    async ({
      ...
      try {
        ...
        const summaries: DashboardSummary[] = await Promise.all(
...
export const deleteDashboardFn = protectedPostFn
  ...
  .handler(async ({ data, context }): Promise<ApiResponse<{ deleted: boolean }>> => {
    try {
      const userId = context.user.id;
